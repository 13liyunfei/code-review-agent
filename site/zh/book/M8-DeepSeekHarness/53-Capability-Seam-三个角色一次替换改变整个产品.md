...
<img class="mermaid-svg" src="/zh/book-assets/diag-0128.svg" alt="⚠️ 事实基准：本讲所有 `路径:行号` 引用，与第 49 讲讲首的模块级声明同源 —— 取自 2026-09-26 实读的 `github.com/deepseek-ai/deepseek-harness`，commit `477b4f4`（标签 `rel/dsh-0.1.7-rc.2`）。路径是&quot;这一版的事实&quot;，不是永久契约（`dsh` 官方自称处于开发者预览并预告破坏性变更）。" />
