 ...
 * 编码规范审查 Agent。
 *
 * <p>审查职责（见文档）：代码风格、命名、注释规范等。
 * 确定性规则（超长行、TODO 标记、行尾空白等）已抽取为注册中心内的
 * {@code PatternSkill}，由本 Agent 实时取用；LLM 负责补充语义级发现。
 */
public class StyleAgent extends AbstractReviewAgent implements ReviewAgent {

    public StyleAgent(LlmClient llmClient,
                      PromptTemplateLoader promptLoader,
                      SkillRegistry registry,
                      ConfidenceCalibrationService calibration,
                      CodeReviewAiService aiService) {
        super(llmClient, promptLoader, registry, calibration, "style", aiService);
    }
    ...
    public boolean supports(List<CodeDiff> diffs, ReviewContext ctx) {
        // 纯文档/配置文件 PR 无代码可审规范，跳过以节省 LLM token
