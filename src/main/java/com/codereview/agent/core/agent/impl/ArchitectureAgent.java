 ...
 * 架构审查 Agent。
 *
 * <p>审查职责（见文档）：分层合理性、耦合度、依赖管理。确定性规则（散落 new Thread、
 * instanceof 类型判断链等）已抽取为注册中心内的 {@code PatternSkill}，由本 Agent
 * 实时取用；LLM 负责补充语义级发现。
 */
public class ArchitectureAgent extends AbstractReviewAgent implements ReviewAgent {
    ...
    public ArchitectureAgent(LlmClient llmClient,
        ...
        super(llmClient, promptLoader, registry, calibration, "architecture", aiService);
    }
    ...
    public boolean supports(List<CodeDiff> diffs, ReviewContext ctx) {
        // 纯文档/配置文件 PR 无代码可审架构语义，跳过以节省 LLM token
    ...
    public List<Finding> review(List<CodeDiff> diffs, ReviewContext ctx) {
        ...
        findings.addAll(llmFindings("architecture_review", defaultVars(diffs, ctx, "架构师")));
        return findings;
    }
}
